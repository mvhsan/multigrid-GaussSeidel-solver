...
with open('intermediary.xml', 'w+') as file:
...
for n in range(num_steps):
    fileid = getfileid(n)
    with open('output_poisson' + fileid + '.vtu', 'w+') as output:
            ...
            if('</Cells>' in line):
                ...
                for i in range(0, len(newmap)):
                    ...
                    if(i != (len(newmap) - 1)):
                        ...
                        line = line + '</DataArray>\n</PointData>\n'
            output.write(line)
with open('output_poisson.pvd', 'w+') as output:
