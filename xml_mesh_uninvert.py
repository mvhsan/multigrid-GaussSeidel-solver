...
with open('tt-sr-mit.tet_mesh.xml') as file:
...
oldfile = open('tt-sr-mit.tet_mesh.xml')
oldfile.seek(0)
#with open('tetmeshFromBlender_uninverted.xml', 'w+') as newfile:
with open('tt-sr-mit.tet_mesh_uninverted.xml', 'w+') as newfile:
    for line in oldfile:
        if '<vertex' in line:
            index = (re.findall('index="([0-9]*)', line)[0])
            newline = '      <vertex index="{0}" x="{1}" y="{2}" z="{3}" />\n'.format(index, Original_vertices[index][0], Original_vertices[index][1], Original_vertices[index][2])
        elif '<tetrahedron' in line:
            index = (re.findall('index="([0-9]*)', line)[0])
            newline = '      <tetrahedron index="{0}" v0="{1}" v1="{2}" v2="{3}" v3="{4}" />\n'.format(index, Tetrahedrons[index][0], Tetrahedrons[index][1], Tetrahedrons[index][2], Tetrahedrons[index][3])
